...
class MatchForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if league:
            self.fields['HOMETEAM'].queryset = Club.objects.filter(LEAGUEPLAYING=league.LEAGUE_ID)
            self.fields['AWAYTEAM'].queryset = Club.objects.filter(LEAGUEPLAYING=league.LEAGUE_ID)
